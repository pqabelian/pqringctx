...
// todo: review the adding of (PublicRand, Tag)
func (pp *PublicParameter) CoinAddressKeyForPKRingGen(coinSpendKeyRandSeed []byte, coinSerialNumberKeyRandSeed []byte, coinDetectorKey []byte, publicRand []byte) (coinAddress []byte, coinSpendSecretKey []byte, coinSerialNumberSecretKey []byte, err error) {
	...
	coinAddress = make([]byte, 1+len(serializedAPK)+len(publicRand)+MACOutputBytesLen)
...
func (pp *PublicParameter) CoinAddressKeyForPKRingVerify(coinAddress []byte, coinSpendSecretKey []byte, coinSerialNumberSecretKey []byte, coinDetectorKey []byte) (bool, error) {
	...
	if coinAddressTypeInAddress == CoinAddressTypePublicKeyForRingPre {
		...
		if len(coinDetectorKey) != MACKeyBytesLen {
		...
		//	check the size
		if len(coinAddress) != 1+pp.addressPublicKeyForRingSerializeSize()+pp.paramKeyGenPublicRandBytesLen+MACOutputBytesLen {
			return false, nil
		}
		...
		//	check the MAC tag
		coinAddressMsg := make([]byte, 1+pp.addressPublicKeyForRingSerializeSize()+pp.paramKeyGenPublicRandBytesLen)
		coinAddressTag := make([]byte, MACOutputBytesLen)
		copy(coinAddressMsg, coinAddress[:1+pp.addressPublicKeyForRingSerializeSize()+pp.paramKeyGenPublicRandBytesLen])
		copy(coinAddressTag, coinAddress[1+pp.addressPublicKeyForRingSerializeSize()+pp.paramKeyGenPublicRandBytesLen:])
...
func (pp *PublicParameter) CoinAddressKeyForPKHSingleGen(coinSpendKeyRandSeed []byte, coinDetectorKey []byte, publicRand []byte) (coinAddress []byte, coinSpendSecretKey []byte, err error) {
	...
	coinAddress = make([]byte, 1+HashOutputBytesLen+len(publicRand)+MACOutputBytesLen)
...
func (pp *PublicParameter) CoinAddressKeyForPKHSingleVerify(coinAddress []byte, coinSpendSecretKey []byte, coinDetectorKey []byte) (bool, error) {
	...
	if coinAddressTypeInAddress == CoinAddressTypePublicKeyHashForSingle {
		...
		//	check the tag
		coinAddressMsg := make([]byte, 1+HashOutputBytesLen+pp.paramKeyGenPublicRandBytesLen)
		coinAddressTag := make([]byte, MACOutputBytesLen)
		copy(coinAddressMsg, coinAddress[:1+HashOutputBytesLen+pp.paramKeyGenPublicRandBytesLen])
		copy(coinAddressTag, coinAddress[1+HashOutputBytesLen+pp.paramKeyGenPublicRandBytesLen:])
		if !MACVerify(coinDetectorKey, coinAddressMsg, coinAddressTag) {
			return false, nil
		}
...
func (pp *PublicParameter) ExtractCoinAddressTypeFromCoinAddress(coinAddress []byte) (CoinAddressType, error) {
	...
	} else if n == 1+pp.addressPublicKeyForRingSerializeSize()+pp.paramKeyGenPublicRandBytesLen+MACOutputBytesLen {
	...
	} else if n == 1+HashOutputBytesLen+pp.paramKeyGenPublicRandBytesLen+MACOutputBytesLen {
