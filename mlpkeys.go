...
func (pp *PublicParameter) CoinAddressKeyForPKHSingleGen(coinSpendKeyRandSeed []byte, coinDetectorKey []byte, publicRand []byte, coinAddressType CoinAddressType) (coinAddress []byte, coinSpendSecretKey []byte, err error) {
	...
	if coinAddressType != CoinAddressTypePublicKeyHashForSingle && coinAddressType != CoinAddressTypePublicKeyHashForSingleCT {
		return nil, nil, fmt.Errorf("CoinAddressKeyForPKHSingleGen: the input coinAddressType (%d) "+
			"is not CoinAddressTypePublicKeyHashForSingle or CoinAddressTypePublicKeyHashForSingleCT", coinAddressType)
	}
	...
	coinAddress[0] = byte(coinAddressType)
	...
	coinSpendSecretKey[0] = byte(coinAddressType)
...
func (pp *PublicParameter) CoinAddressKeyForPKHSingleVerify(coinAddress []byte, coinSpendSecretKey []byte, coinDetectorKey []byte, coinAddressType CoinAddressType) (bool, error) {
	...
	if coinAddressType != CoinAddressTypePublicKeyHashForSingle && coinAddressType != CoinAddressTypePublicKeyHashForSingleCT {
		return false, fmt.Errorf("CoinAddressKeyForPKHSingleVerify: the input coinAddressType(%d) "+
			"is not CoinAddressTypePublicKeyHashForSingle or CoinAddressTypePublicKeyHashForSingleCT", coinAddressType)
	}
	...
	//	match check
	if coinAddressTypeInAddress != coinAddressType {
		return false, fmt.Errorf("CoinAddressKeyForPKHSingleVerify: the coinAddressType (%d) of the input (coinAddress, coinSpendSecretKey) "+
			"does not match the input coinAddressType (%d)",
			coinAddressTypeInAddress, coinAddressType)
	}
	...
	if len(coinDetectorKey) != pp.GetParamMACKeyBytesLen() {
		return false, fmt.Errorf("CoinAddressKeyForPKHSingleVerify: the input coinDetectorKey has an invalid length (%d)",
			len(coinDetectorKey))
	}
...
func (pp *PublicParameter) CoinAddressForPKHSingleDetect(coinAddress []byte, coinDetectorKey []byte) (bool, error) {
	...
	//	match check
	if coinAddressTypeInAddress != CoinAddressTypePublicKeyHashForSingle && coinAddressTypeInAddress != CoinAddressTypePublicKeyHashForSingleCT {
		return false, fmt.Errorf("CoinAddressForPKHSingleDetect: the coinAddressType (%d) of the input coinAddress is not "+
			"CoinAddressTypePublicKeyHashForSingle or CoinAddressTypePublicKeyHashForSingleCT", coinAddressTypeInAddress)
	}
...
func (pp *PublicParameter) ExtractCoinAddressTypeFromCoinSerialNumberSecretKey(coinSnSecretKey []byte) (CoinAddressType, error) {
	...
	if n == pp.addressSecretKeySnSerializeSize() {
		...
		return 0, fmt.Errorf("ExtractCoinAddressTypeFromCoinSerialNumberSecretKey: attempting extracting CoinAddressType from a nil/empty CoinSerialNumberSecretKey")
	}
...
func (pp *PublicParameter) coinSpendSecretKeyForPKRingParse(coinSpendSecretKey []byte) (*AddressSecretKeySp, error) {
	...
	case CoinAddressTypePublicKeyHashForSingleCT:
		return nil, fmt.Errorf("coinSpendSecretKeyForPKRingParse: the coinAddressType of input coinSpendSecretKey is CoinAddressTypePublicKeyHashForSingleCT")
...
func (pp *PublicParameter) coinSerialNumberSecretKeyForPKRingParse(coinSerialNumberSecretKey []byte) (*AddressSecretKeySn, error) {
	...
	case CoinAddressTypePublicKeyHashForSingleCT:
		return nil, fmt.Errorf("coinSerialNumberSecretKeyForPKRingParse: the coinAddressType of input coinSerialNumberSecretKey is CoinAddressTypePublicKeyHashForSingleCT")
	default:
...
func (pp *PublicParameter) coinSpendSecretKeyForPKHSingleParse(coinSpendSecretKey []byte) (*AddressPublicKeyForSingle, *AddressSecretKeySp, error) {
	...
	if coinAddressType != CoinAddressTypePublicKeyHashForSingle && coinAddressType != CoinAddressTypePublicKeyHashForSingleCT {
		return nil, nil, fmt.Errorf("coinSpendSecretKeyForPKHSingleParse: the coinAddressType (%d) of input coinSpendSecretKey is "+
