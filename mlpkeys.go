...
func (pp *PublicParameter) CoinAddressKeyForPKHSingleGen(coinSpendKeyRandSeed []byte, coinDetectorKey []byte, publicRand []byte, coinAddressType CoinAddressType) (coinAddress []byte, coinSpendSecretKey []byte, err error) {
	...
	if coinAddressType != CoinAddressTypePublicKeyHashForSingle && coinAddressType != CoinAddressTypePublicKeyHashForSingleCT {
		return nil, nil, fmt.Errorf("CoinAddressKeyForPKHSingleGen: the input coinAddressType (%d) is not CoinAddressTypePublicKeyHashForSingle or CoinAddressTypePublicKeyHashForSingleCT", coinAddressType)
	}
...
func (pp *PublicParameter) CoinAddressKeyForPKHSingleVerify(coinAddress []byte, coinSpendSecretKey []byte, coinDetectorKey []byte, coinAddressType CoinAddressType) (bool, error) {
	...
	if coinAddressType != CoinAddressTypePublicKeyHashForSingle && coinAddressType != CoinAddressTypePublicKeyHashForSingleCT {
		return false, fmt.Errorf("CoinAddressKeyForPKHSingleVerify: the input coinAddressType(%d) is not CoinAddressTypePublicKeyHashForSingle or CoinAddressTypePublicKeyHashForSingleCT", coinAddressType)
	}
	...
	if coinAddressTypeInAddress != coinAddressType {
		return false, fmt.Errorf("CoinAddressKeyForPKHSingleVerify: the coinAddressType (%d) of the input (coinAddress, coinSpendSecretKey) does not match the input coinAddressType (%d)",
